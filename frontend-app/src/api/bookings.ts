...
export async function getBookingById(bookingId: number | string): Promise<Booking> {
  const booking = await apiRequest<unknown>(`/api/bookings/${bookingId}`, {
    auth: demoGuestAuth
  });
...
export async function cancelBooking(bookingId: number | string): Promise<Booking> {
  const booking = await apiRequest<unknown>(`/api/bookings/${bookingId}/cancel`, {
    auth: demoGuestAuth,
    method: "POST"
  });

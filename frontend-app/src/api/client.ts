...
function buildAuthorizationHeader(auth: RequestOptions["auth"]): string | undefined {
  if (!auth) {
    return undefined;
  }
...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  if (normalized.data === null || normalized.data === undefined) {
    return undefined as T;
  }
    ...
    try {
      const response = await fetch(`${env.apiBaseUrl}${path}`, {
        ...
        headers: {
          ...
          ...(buildAuthorizationHeader(options.auth) ? { Authorization: buildAuthorizationHeader(options.auth) } : {}),
          ...options.headers
        },

...
\hline
Layered Architecture & \texttt{controller}, \texttt{service}, \texttt{repository}, \texttt{entity} & Separates API handling, business logic, data access, and domain representation. \\
\hline
Repository Pattern & \texttt{VillaRepository}, \texttt{BookingRepository}, \texttt{PaymentRepository}, \texttt{UserRepository} & Hides database query details and allows services to work through clean repository interfaces. \\
\hline
DTO \& Mapper Pattern & \texttt{dto} and \texttt{mapper} packages & Prevents direct exposure of entities through APIs and centralizes request/response conversion. \\
\hline
Strategy Pattern & \texttt{PaymentStrategy}, \texttt{MomoPaymentStrategy}, \texttt{VNPayPaymentStrategy} & Separates payment behavior by provider and makes it easier to add new providers. \\
\hline
Factory Pattern & \texttt{PaymentStrategyFactory} & Selects the correct payment strategy by provider and keeps \texttt{PaymentService} decoupled from concrete classes. \\
\hline
\end{longtable}

\subsection{Design Pattern UML Note}

PlantUML files for the design patterns are available in \texttt{docs/uml}: \texttt{design-pattern-layered-architecture.puml}, \texttt{design-pattern-repository.puml}, \texttt{design-pattern-dto-mapper.puml}, \texttt{design-pattern-strategy.puml}, and \texttt{design-pattern-factory.puml}. They can be exported as images and inserted into the final PDF if required.
...
The implementation covers the main use cases from UC-01 to UC-14. Important rules are implemented, including overlapping booking prevention, guest count validation, guest ownership checks for booking and payment actions, payment statuses PENDING/SUCCESSFUL/FAILED, and valid booking status transitions.

\subsection{Testing}

According to the implementation overview, backend integration tests completed successfully:
...
\section*{Remaining Items Before Submission}

\begin{itemize}[noitemsep]
  \item Export UML images from the PlantUML files in \texttt{docs/uml}.
  \item Add any required cover-page or formatting details if requested by the lecturer.
\end{itemize}
